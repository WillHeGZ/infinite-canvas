...
- **支持**：文生图、图生图（参考图压缩至 1024 边长、服务繁忙 503 自动重试）、图片反推、视频生成（帧数遵循 `8n+1` 规则、按分辨率与横竖屏自动推导宽高、60 秒长间隔轮询并容忍限流）
- **图片档位**：图片请求使用官方档位表示法（`size: "1K"/"2K"/"3K"/"4K"` + `ratio` 八值白名单），由画质设置与比例选项自动映射（low→1K、auto/medium→2K、high→3K）。网关按档位限流（如 3K 档约 1 请求/分钟），触发限流时会解析错误消息中的窗口时长并自动等待重试（最多 6 次），等待期间取消生成立即生效
